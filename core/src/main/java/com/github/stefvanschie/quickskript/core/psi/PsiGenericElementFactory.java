 ...
 * used for parsing by this class must have a mandatory {@link TypeRegistry.Entry}[] parameter, after the optional
 * {@link SkriptMatchResult} parameter and before the first parameter corresponding to the first type (or line number if
 * no types exist or the method is annotated with {@link Fallback}). If the type is irrelevant, the
 * {@link TypeRegistry.Entry}[] argument may be null.
 * <p>
