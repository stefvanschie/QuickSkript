...
public class PsiInverseSineFunction extends PsiElement<Double> {
     ...
     * @since 0.1.0
     */
    private PsiInverseSineFunction(@NotNull PsiElement<?> parameter, int lineNumber) {
        super(lineNumber);
    ...
    public static class Factory implements PsiElementFactory {
         ...
         */
        @NotNull
        private final Pattern pattern = Pattern.compile("asin\\((?<parameter>[\\s\\S]+)\\)");
        ...
        @Contract(pure = true)
        @Fallback
        public PsiInverseSineFunction tryParse(@NotNull SkriptLoader skriptLoader, @NotNull String text, int lineNumber) {
        ...
        @NotNull
        @Contract(pure = true)
        protected PsiInverseSineFunction create(@NotNull PsiElement<?> element, int lineNumber) {
