...
public class PsiTangentFunction extends PsiElement<Double> {
     ...
     * @since 0.1.0
     */
    private PsiTangentFunction(@NotNull PsiElement<?> parameter, int lineNumber) {
        super(lineNumber);
    ...
    public static class Factory implements PsiElementFactory {
         ...
         */
        @NotNull
        private final Pattern pattern = Pattern.compile("tan\\((?<parameter>[\\s\\S]+)\\)");
        ...
        @Contract(pure = true)
        @Fallback
        public PsiTangentFunction tryParse(@NotNull SkriptLoader skriptLoader, @NotNull String text, int lineNumber) {
        ...
        @NotNull
        @Contract(pure = true)
        protected PsiTangentFunction create(@NotNull PsiElement<?> element, int lineNumber) {
